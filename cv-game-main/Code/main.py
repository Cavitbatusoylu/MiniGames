...
class Game:
    def __init__(self):
        ...
        self.lives = 3
        self.live_surf = pygame.image.load(resource_path('player.png')).convert_alpha()
        self.score = 0
        self.font = pygame.font.Font(resource_path('Pixeled.ttf'), 14)
        self.small_font = pygame.font.Font(resource_path('Pixeled.ttf'), 7)
        ...
        # Audio
        self.laser_sound = pygame.mixer.Sound(resource_path('laser.wav'))
...
def start_game():
    ...
    while True:
        ...
        try:
            ...
            frame_scaled = pygame.transform.scale(frame_rgb, (int(video_width), int(video_height)))
        ...
        logo = pygame.image.load(os.path.join(os.path.dirname(__file__), '..', 'Resources', 'logo.png')).convert_alpha()
        logo = pygame.transform.scale(logo, (window_width / window_width * 400, window_height / window_height * 100))
        screen.blit(logo, (-22, 6))

        move0 = pygame.image.load(os.path.join(os.path.dirname(__file__), '..', 'Resources', 'move0.png')).convert_alpha()
        move0 = pygame.transform.scale(move0, (window_width / window_width * 330, window_height / window_height * 90))
        screen.blit(move0, (17, 210))

        shoot0 = pygame.image.load(os.path.join(os.path.dirname(__file__), '..', 'Resources', 'shoot0.png')).convert_alpha()
        ...
        flip0 = pygame.image.load(os.path.join(os.path.dirname(__file__), '..', 'Resources', 'flip0.png')).convert_alpha()
...
if __name__ == '__main__':
    ...
    screen = pygame.display.set_mode((info.current_w, info.current_h), pygame.FULLSCREEN | pygame.SCALED | pygame.HWSURFACE | pygame.DOUBLEBUF)
    pygame.display.set_caption("Gesture-Controlled Space Invaders+")

    clock = pygame.time.Clock()

    # Recompute layout based on actual fullscreen size so camera and game fill the screen
    window_width, window_height = screen.get_size()
    screen_width = int(window_width * 0.62)  # game panel width (right side)
    video_width = int(window_width - screen_width)  # camera panel width (left side)
    video_height = int(window_height)
    video_pos_x = 0
    video_pos_y = 0
