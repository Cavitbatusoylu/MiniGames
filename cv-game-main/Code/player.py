...
class Player(pygame.sprite.Sprite):
    def __init__(self, pos, cwidth, speed, vwidth, screen, height):
        super().__init__()
        self.image0 = pygame.image.load(os.path.join(os.path.dirname(__file__), '..', 'Resources', 'player.png')).convert_alpha()
        ...
        self.laser_sound = pygame.mixer.Sound(os.path.join(os.path.dirname(__file__), '..', 'Resources', 'laser.wav'))
    ...
    def read_fingers(self):
        ...
        result = self.detector.findHands(img, draw=False, flipType=False)
        hands = None
        out_img = img
        if isinstance(result, tuple) and len(result) == 2:
            # cvzone returns either (hands, img) or (img, hands)
            a, b = result
        ...
        if self.hands:
            hand = self.hands[0]
            # Güvenli bbox çıkarımı
            x = y = w = h = 0
            try:
                if isinstance(hand, dict) and 'bbox' in hand:
                    x, y, w, h = hand['bbox']
                else:
                    # lmList'ten bbox hesapla
                    lm = hand['lmList'] if isinstance(hand, dict) and 'lmList' in hand else None
                    if lm:
                        xs = [p[0] for p in lm]
                        ys = [p[1] for p in lm]
                        ...
                        # merkez varsa merkezden tahmini bbox
                        cxcy = hand['center'] if isinstance(hand, dict) and 'center' in hand else (self.rect.centerx, self.rect.centery)
                        x, y, w, h = int(cxcy[0] - 50), int(cxcy[1] - 50), 100, 100
            except Exception:
                x, y, w, h = 0, 0, 0, 0
            x1 = x + w // 2
    ...
    def update(self):
        if self.read_fingers():
            ...
            if self.fingers[1] == 1:
                shoot1 = pygame.image.load(os.path.join(os.path.dirname(__file__), '..', 'Resources', 'shoot1.png')).convert_alpha()
                ...
                if self.game_state == 2 or self.game_state == 4:
                    self.game_state = 3
                flip1 = pygame.image.load(os.path.join(os.path.dirname(__file__), '..', 'Resources', 'flip1.png')).convert_alpha()
                flip1 = pygame.transform.scale(flip1,(self.ww / self.ww * 330, self.wh / self.wh * 90))
                self.screen.blit(flip1, (17, 400))
            else:
                move1 = pygame.image.load(os.path.join(os.path.dirname(__file__), '..', 'Resources', 'move1.png')).convert_alpha()
